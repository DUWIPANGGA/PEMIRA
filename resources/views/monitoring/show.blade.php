...
@endsection
@section('content')
    <main class="w-4/4 bg-gray-100 p-6 rounded-lg shadow">
        ...
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-center items-center flex-wrap gap-8">
                @foreach ($teams->candidates as $index => $candidate)
                    ...
                    @endphp
                    <div class="text-white shadow-lg rounded-lg p-6 w-64 text-center" style="background: linear-gradient(to right, {{ $bg[0] }}, {{ $bg[1] }});">
                        <img src="{{ asset('image/icon/user.png') }}" alt="Paslon" class="w-32 h-32 object-cover rounded-full shadow-xl mb-4 mx-auto border-4 border-white">
                        <p class="font-semibold text-xl mb-2">{{ $candidate->name }}</p>
                        <p class="text-sm mb-4">Total Suara: {{ $candidate->votes_count }}</p>
                        <span class="bg-white text-gray-800 px-6 py-2 rounded-full font-semibold shadow-md">{{ $candidate->votes_count }} Suara</span>
                    </div>
                @endforeach
            </div>
        </div>
