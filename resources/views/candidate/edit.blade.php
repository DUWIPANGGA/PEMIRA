...
@section('content')
    ...
    <div class="bg-white shadow rounded-2xl p-8 border border-gray-200">
        <div class="text-center mb-8">
            <h1 class="text-2xl font-bold text-gray-900">Form Edit Calon</h1>
            <p class="text-sm text-gray-500 mt-2">Silakan ubah detail calon kandidat di bawah ini</p>
        </div>
        ...
        <form action="{{ route('candidate.update', $candidate->id) }}" method="POST" enctype="multipart/form-data"
            ...
            @csrf
            @method('PUT')
            <input type="hidden" name="events_id" value="{{ $candidate->events_id }}">
            ...
            <div>
                ...
                <textarea name="name" id="name" rows="4" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring focus:ring-blue-300">{{ old('name', $candidate->name) }}</textarea>
            </div>

            <!-- Slogan -->
            <div>
                <label for="slogan" class="block text-sm font-medium text-gray-700 mb-1">Slogan</label>
                <textarea name="slogan" id="slogan" rows="4" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring focus:ring-blue-300">{{ old('slogan', $candidate->slogan) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="vision" id="vision" rows="4" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring focus:ring-blue-300">{{ old('vision', $candidate->vision) }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="mission" id="mission" rows="4" required
                    class="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring focus:ring-blue-300">{{ old('mission', $candidate->mission) }}</textarea>
            </div>
            <!-- Pilih Ketua -->
            <div>
                <label for="ketua" class="block text-sm font-medium text-gray-700 mb-1">Pilih Ketua</label>
                <select name="ketua" id="ketua" class="select2 w-full" required>
                    <option value="">-- Pilih Calon Ketua --</option>
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('ketua', $ketua) == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="wakilketua" id="wakil-ketua" required
                    ...
                    @foreach ($users as $user)
                        <option value="{{ $user->id }}" {{ old('wakilketua', $wakil) == $user->id ? 'selected' : '' }}>
                            {{ $user->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                <button type="submit"
                    class="w-full py-3 px-6 text-white bg-blue-600 rounded-md font-semibold shadow hover:bg-blue-700 transition">
                    Simpan Perubahan
                </button>
            </div>
        </form>
